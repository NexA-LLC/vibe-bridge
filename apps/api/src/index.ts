...
const fetchJobRecord = async (jobId: string): Promise<JobRecord | null> => {
  if (!useDb) return jobs.get(jobId) || null;
  const db = await ensureDb();
  if (!db) return null;
  ...
  return mapJobRow(rows[0], eventRows.map(mapEventRow));
};
...
const listJobsSummary = async (filters: {
  ...
  let query = db.select().from(jobsTable).orderBy(desc(jobsTable.updatedAt)).limit(filters.limit).$dynamic();
  ...
  for (const entry of counts) countMap.set(entry.jobId, Number(entry.count));
  return rows.map((row) => ({
    id: row.id,
    tenantId: row.tenantId,
    kind: row.kind,
